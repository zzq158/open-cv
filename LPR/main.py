...
def read_photo(filename,flags=cv2.IMREAD_COLOR):
    """
    该函数用于读取磁盘中的图片文件，默认以彩色方式进行读取
    输入：filename 指的是图像的文件名
         flags 用来表示按照什么方式读取图片
                IMREAD_COLOR 彩色图像
...
def predict(imageArr):
    """
    这个函数通过一系列的处理，找到可能是车牌的一些矩形区域,这些区域存放在轮廓中
...
if __name__ == "__main__":
    ...
    license_segment(car_plate,img_resize)

    # print(img_resize)
    # r, g, b = cv2.split(img_resize)  # 进行将图片通道拆分
    # print(r)
    # print(r.shape)
    #cv2.imshow('img_resize',img_resize)
    # cv2.imshow('gray_img', gray_img)
    #gray_img_resize = resize_photo(gray_img)
    #print(gray_img_resize)
    # cv2.imshow('gray_img_resize',gray_img_resize)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()